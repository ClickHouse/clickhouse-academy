...
SELECT tzl.borough, tzl.zone, count() FROM $TABLE AS nyct JOIN taxi_zone_lookup_opt6_2 AS tzl ON nyct.pickup_location_id = tzl.id GROUP BY tzl.borough, tzl.zone ORDER BY 3 DESC LIMIT 10
...
SELECT avg(fare_amount), avg(trip_distance) FROM taxi_zone_lookup_opt6_2 JOIN $TABLE ON pickup_location_id = taxi_zone_lookup_opt6_2.id WHERE taxi_zone_lookup_opt6_2.zone ILIKE '%airport%'
...
SELECT ride_date, max_fare, dropoff_id, tzl.borough, tzl.zone FROM (SELECT toDate(pickup_datetime) AS ride_date, max(fare_amount) AS max_fare, argMax(dropoff_location_id, fare_amount) AS dropoff_id FROM $TABLE GROUP BY toDate(pickup_datetime)) AS max_fares JOIN taxi_zone_lookup_opt6_2 AS tzl ON max_fares.dropoff_id = tzl.id ORDER BY ride_date ASC
