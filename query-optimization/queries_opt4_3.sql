...
SELECT tzl.borough, tzl.zone, count() FROM $TABLE AS nyct JOIN taxi_zone_lookup_opt4_3 AS tzl ON nyct.pickup_location_id = tzl.id GROUP BY tzl.borough, tzl.zone ORDER BY 3 DESC LIMIT 10
...
SELECT avg(fare_amount), avg(trip_distance) FROM taxi_zone_lookup_opt4_3 JOIN $TABLE ON pickup_location_id = taxi_zone_lookup_opt4_3.id WHERE taxi_zone_lookup_opt4_3.zone ILIKE '%airport%'
